...
class LLMAgent: 
    ...
    def get_list_of_instructions(self, message):
        system_msg = "You are an LLM agent that helps user perform google tasks based on their instructions. \
            ...
            Separate the calls into one action in each line as a sentence. \
            ...
            Create a new calendar event tomorrow at 9am with name 'meeting'. ".format(get_current_time())
    ...
    def get_service_code(self, message):
        system_msg = "You are an LLM agent that helps user generate function calls to Google API. \
            Based on the user's desired action on Google account, return a piece of code using Google HTTP API to perform the user-specified action. \
    ...
    def get_service_scope(self, message):
        system_msg = "You are an LLM agent that helps user generate function calls to Google Python Client Library. \
            ...
            Do not give instructions, do not format the output, do not include http params, just a plain Google API scope string. \
            Example prompt: Create a calendar event tomorrow at 9am. \
...
if __name__ == '__main__':
    ...
    try:
        ...
        match api_calls[0].method:
            case 'GET':
                response = requests.get(api_calls[0].api, params=api_calls[0].params, headers=api_calls[0].headers, data=api_calls[0].body)
            case 'POST':
                response = requests.post(api_calls[0].api, params=api_calls[0].params, headers=api_calls[0].headers, data=api_calls[0].body)
            case 'PUT':
                response = requests.put(api_calls[0].api, params=api_calls[0].params, headers=api_calls[0].headers, data=api_calls[0].body)
            case 'DELETE':
                response = requests.delete(api_calls[0].api, params=api_calls[0].params, headers=api_calls[0].headers, data=api_calls[0].body)
